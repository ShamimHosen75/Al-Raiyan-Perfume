...
import { Loader2, Plus, Shield, ShieldAlert, ShieldCheck, UserCheck, Users, UserX } from 'lucide-react';
...
export default function AdminUsers() {
  ...
  const fetchUsers = async () => {
    // Direct DB query instead of Edge Function for local reliability
  ...
  const updateRoleMutation = useMutation({
    mutationFn: async ({ user_id, role }: { user_id: string; role: string }) => {
      const { data, error } = await supabase.functions.invoke('manage-users?action=update-role', {
        body: { user_id, role },
      });
      if (error) throw error;
      return data;
  ...
  const toggleActiveMutation = useMutation({
    mutationFn: async ({ user_id, is_active }: { user_id: string; is_active: boolean }) => {
      const { data, error } = await supabase.functions.invoke('manage-users?action=toggle-active', {
